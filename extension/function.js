...
async function sha256Fast(value) {
    // 如果支持 crypto.subtle，则优先使用crypto.subtle，否则使用sha256函数
...
const AES = {
    encrypt: async (value, key, iv) => {
        // 如果支持 crypto.subtle，则优先使用crypto.subtle，否则使用 aes 函数
        if (crypto?.subtle?.encrypt) {
            ...
            const encrypted = await crypto.subtle.encrypt(
                ...
                await crypto.subtle.importKey('raw', keyBuffer, 'AES-CBC', false, ['encrypt']),
...
export async function upload_cookie(payload) {
    ...
    const data = { "cookie_data": cookies, "local_storage_data": local_storages, "update_time": new Date() }
    const encrypted = await cookie_encrypt(uuid, data, password);
    const data_to_encrypt = JSON.stringify(encrypted)
    const endpoint = payload['endpoint'].trim().replace(/\/+$/, '') + '/update';

    // get sha256 of the encrypted data
    const sha256 = await sha256Fast(uuid + "-" + password + "-" + endpoint + "-" + data_to_encrypt);
    ...
    // 加密用的算法来自 crypto 还是 crypto-js
    const type = crypto?.subtle?.encrypt ? 'crypto' : 'crypto-js'
...
async function cookie_encrypt(uuid, data, password) {
    const the_key = md5(uuid + '-' + password).toString().substring(0, 16);
    const iv = new Uint8Array(16).fill(0)
    const encrypted = (await AES.encrypt(JSON.stringify(data), the_key, iv))
    return encrypted;
}
